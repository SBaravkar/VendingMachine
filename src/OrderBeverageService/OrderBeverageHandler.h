...
void OrderBeverageServiceHandler::PlaceOrder(std::string& _return, const int64_t city){
  ...
  // by default get beverage as drink
  std::string beverage = "drink";

  // 5. call the remote procedure : GetBeverageType
  try {
...
// 6. business logic 
std::string result = "Enjoy your " + beverage + " on this " + (weatherType == WeatherType::type::WARM ? "warm" : "cold") + " day!";
_return = result;
